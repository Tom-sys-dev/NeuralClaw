...
# Packages dont l'installation est interdite (sécurité serveur)
_IDE_BLOCKED_PACKAGES = {
    "ansible", "fabric", "paramiko",   # accès SSH
    "scapy", "nmap",                   # scan réseau
    "pwntools",                        # exploitation
}

IDE_INSTALL_TIMEOUT = 120


def _sanitize_package_name(name: str) -> tuple[bool, str]:
    """Vérifie qu'un nom de package est valide (PEP 508 simplifié)."""
    import re as _re
    # Accepte : nom, nom==1.0, nom>=2, nom[extra], nom @ url basique
    if not _re.match(r'^[A-Za-z0-9_\-\.\[\]<>=!@~]+$', name):
        return False, f"Nom de package invalide : {name!r}"
    base = _re.split(r'[>=<!@\[]', name)[0].lower().replace("-", "_")
    if base in _IDE_BLOCKED_PACKAGES:
        return False, f"Package non autorisé : {base}"
    return True, ""


@ide_bp.route("/api/ide/install", methods=["POST"])
def ide_install():
    """Installe un ou plusieurs packages pip."""
    data     = request.get_json(force=True, silent=True) or {}
    packages = data.get("packages", [])
    if isinstance(packages, str):
        packages = [p.strip() for p in packages.split(",") if p.strip()]
    if not packages:
        return jsonify({"error": "Aucun package spécifié"}), 400
    if len(packages) > 20:
        return jsonify({"error": "Maximum 20 packages à la fois"}), 400

    for pkg in packages:
        valid, reason = _sanitize_package_name(pkg)
        if not valid:
    ...
    try:
        ...
        # Extrait les noms effectivement installés
        import re as _re
        installed = _re.findall(r"Successfully installed (.+)", output)
        installed_list: list[str] = []
        for match in installed:
            installed_list.extend(match.split())

        return jsonify({
            "output":      output[:IDE_MAX_OUTPUT_LENGTH],
            "returncode":  proc.returncode,
            "duration_ms": duration_ms,
            "installed":   installed_list,
        })
    except subprocess.TimeoutExpired:
        return jsonify({"error": f"Timeout : installation dépassée ({IDE_INSTALL_TIMEOUT}s)"}), 408
    except Exception as exc:
        logger.error("Erreur installation IDE : %s", exc)
        return jsonify({"error": str(exc)}), 500


@ide_bp.route("/api/ide/uninstall", methods=["POST"])
def ide_uninstall():
    """Désinstalle un package pip."""
    data = request.get_json(force=True, silent=True) or {}
    pkg  = (data.get("package") or "").strip()
    if not pkg:
        return jsonify({"error": "Package non spécifié"}), 400
    valid, reason = _sanitize_package_name(pkg)
    if not valid:
        return jsonify({"error": reason}), 400
    try:
        ...
        return jsonify({"error": "Timeout désinstallation"}), 408
    except Exception as exc:
        return jsonify({"error": str(exc)}), 500
