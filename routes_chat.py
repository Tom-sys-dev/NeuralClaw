...
def chat():
    ...
    # ── Langue ───────────────────────────────────────────────────────────
    # BUG FIX : `data` peut être None si la requête n'est pas JSON
    ...
    while search_count < max_search_attempts:
        ...
        try:
            response = requests.post(
                OPENROUTER_URL, headers=_get_headers(), json=payload, timeout=90
            )
        ...
        if "error" in result:
            ...
            sess["messages"].pop()
            return jsonify({
                "error": f"IA indisponible ({err.get('code', '?')}): {err.get('message', '')}"
            }), 502
        ...
        if reply.strip().startswith("\\recherche "):
            ...
            if not query:
                # BUG FIX : éviter une boucle infinie si la requête est vide
                final_reply = reply
